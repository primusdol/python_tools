...
note.py -s searchstring [searchstring] ..  search items from the saved snippets
...
def newnote(args):
  '''
  no search items were found in the arguments
  so append a new snippet in the file
  '''
...
def searchsnippet(args, snippet):
  '''
  check if all search items exists in this snippet
  '''
  hit = []
  line_cnt = [0, ]
...
def searchfile(args):
  '''
  break the file into snippets
  a snippet starts on a newline with ### followed by a datestamp
  '''
  snippet = []
  cnt = 0
  with open(args.file, 'rb') as fh:
    for line in fh:
      ...
      try:
        ...
        line = f'problem decoding to utf-8 in file {args.file} line {cnt} {e}'
        print(line)
